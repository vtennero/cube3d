...
/*   Updated: 2024/09/29 16:44:37 by cliew            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_enemy_validity(t_game *game, int i, int *texture_index,
		t_texture **enemy_texture)
{
	*texture_index = game->enemies[i].current_frame;
...
void	process_enemy(t_game *game, int i, float center_x, float center_y)
{
	t_sprite_render_context	ctx;
	int						texture_index;
	t_texture				*enemy_texture;

	check_enemy_validity(game, i, &texture_index, &enemy_texture);
	if (!enemy_texture)
	{
		return ;
	}
	if (!init_sprite_render_context(&ctx, game, game->enemies[i].position,
			enemy_texture))
	{
		printf("Error: Failed to initialize sprite render context for \
		 enemy %d\n", i);
		return ;
	}
...
void	check_enemy_at_center(t_game *game)
{
	...
	while (i < game->num_enemies)
	{
		...
		{
			process_enemy(game, i, center_x, center_y);
		}
		i++;
	}
}
...
// 					game->enemies[i].position, enemy_texture))
// 			{
