...
void draw_vertical_slice(void *mlx_ptr, void *win_ptr, char *img_data, int img_width, int img_height, int bpp, int size_line, int screenX, int drawStart, int drawEnd) {
    for (int screenY = drawStart; screenY < drawEnd; screenY++) {
        int texX = (int)(screenX * img_width / 800.0) % img_width;
        int texY = (int)((screenY - drawStart) * img_height / (drawEnd - drawStart)) % img_height;
...
int mlx_test_xpm_to_pixels_scaled_w_perspective_split(void)
{
    ...
    for (int screenX = 0; screenX < 800; screenX++) {
        ...
        draw_vertical_slice(mlx_ptr, win_ptr, img_data, img_width, img_height, bpp, size_line, screenX, drawStart, drawEnd);
    }
