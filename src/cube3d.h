...
/*   Updated: 2024/09/25 16:59:14 by cliew            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define PLAYER_B_HEIGHT 0.2f
# define STRIKE_SHAKE_DURATION 30
# define STRIKE_SHAKE_INTENSITY 0.3f
# define STRIKE_BARRAGE_SCALE 2.0f
...
typedef void			(*t_script_func)(t_game *game);
...
double					calculate_initial_tex_pos(t_ray *ray, t_game *game,
							double step);
double					calculate_tex_step(t_texture *texture, double line_h);
float					calculate_angle(t_game *game);
float					calculate_distance(t_vector2d point1,
							t_vector2d point2);
float					calculate_distance_to_player(t_game *game,
							t_vector2d position);
float					calculate_dynamic_buffer(t_player *player,
							float base_speed);
float					calculate_floor_distance(int p, float player_height);
float					calculate_new_position_x(t_game *game, float dx,
							float bump_distance);
float					calculate_new_position_y(t_game *game, float dy,
							float bump_distance);
float					calculate_screen_shake(t_game *game, int current_frame);
float					calculate_sprite_tex_x(t_sprite_render_context *ctx,
							int stripe);
...
int						assign_texture(char **words, char **path,
							char *texture);
...
int						calculate_floor_pixel_color(t_game *game,
							t_vector2d floor);
int						calculate_floor_start(t_game *game);
int						calculate_frame(long elapsed_microseconds,
							int max_textures);
...
int						check_collision_at_point(t_game *game, t_vector2d dir,
...
int						check_map_error(int *map_start, int word_count,
							char **words);
int						check_order_in_column(int **array, int col,
							int row_count, int direction);
int						check_order_in_row(int **array, int row, int col_count,
							int direction);
int						check_player_position_helper(t_game *game, int i, int j,
							int *player_found);
int						check_player_postion_and_map_char(t_game *game);
int						check_wall_collision(t_game *game, t_enemy *enemy,
							t_vector2d dir);
...
int						get_floor_texture_color(t_texture *texture, int x,
							int y);
int						get_floor_texture_index(t_game *game, int cell_x,
							int cell_y);
int						get_last_three_digit_indexes(char *path, int *f, int *s,
							int *t);
...
int						get_next_eagle_frame(t_strike *strike,
							int offset_index);
int						get_next_enemy_frame(t_enemy *enemy);
int						get_next_napalm_frame(t_strike *strike,
							int offset_index);
int						get_pixel_color(t_sprite_render_context *ctx,
							t_vector2d tex);
int						get_texture_color(t_texture *texture, int x, int y);
int						gun_texture_frame_update(t_game *game, int frame,
							char *digits, char *path);
...
int						is_space_to_wall_transition(t_game *game, t_ray *ray,
							int prev_map_x, int prev_map_y);
int						is_sprite_in_front(float transformY, int stripe,
							int screen_width);
...
int						normalize_direction(float *dirX, float *dirY,
							float length);
int						parse_char_to_int(char chars);
int						parse_floor_sky_rgb(t_game *game);
int						parse_line_to_map_array(char *line, t_game *game,
							int *map_line);
...
int						validate_digits(const char *path, int first, int second,
							int third);
int						validate_game_state(t_game *game);
int						validate_map(int *valid, int **array, int row_count,
							int col_count);
int						validate_path_format(const char *path, int len,
							int min_len);
long					get_elapsed_microseconds(struct timeval *start,
							struct timeval *end);
...
t_vector2d				calc_barrage_new_pos(t_vector2d base_position,
							t_vector2d offset);
t_vector2d				calculate_floor_coordinates(t_game *game,
							t_ray_node *center_ray);
t_vector2d				calculate_mouse_delta(t_vector2d current_pos,
							t_vector2d center);
t_vector2d				calculate_raw_floor_coordinates(t_game *game,
							t_vector2d ray_dir, float rowdist);
t_vector2d				calculate_tile_position(t_vector2d base,
							t_vector2d perp, int i);
t_vector2d				clamp_barr_pos(t_vector2d position, int map_width,
							int map_height);
t_vector2d				clamp_coordinates_to_map(t_vector2d coords, t_map *map);
t_vector2d				generate_random_offset(t_game *game,
...
void					add_script(t_game *game, t_script_func func,
							int delay_seconds);
void					add_script_to_manager(t_game *game,
							t_script *new_script, int slot);
void					adjust_barrage_sprite_dimensions(t_sprite_calc *calc);
void					adjust_momentum(t_enemy *enemy);
void					adjust_ray_position(t_ray *ray, int *prev_map_x,
							int *prev_map_y);
void					apply_barrage_screen_shake(t_game *game,
							int current_frame);
void					apply_napalm_screen_shake(t_game *game,
							int first_visible_frame);
void					apply_reduced_bump(t_game *game, float dx, float dy,
							float *bump_distance);
void					barrage_inbound(t_game *game);
void					bump_player(t_game *game, int enemy_index);
void					calc_barr_sprite_pos(t_game *game, t_vector2d *pos,
							t_vector2d *sprite);
...
void					calc_e_dir(t_game *game, t_enemy *enemy,
							t_vector2d *dir);
...
void					calculate_and_print_center_ray(t_game *game,
							t_ray_node *center_ray);
...
void					calculate_floor_ray_params(t_game *game,
							t_floor_ray *floor_ray, int y);
void					calculate_gun_position(t_game *game,
							t_texture *gun_texture, t_vector2d *start);
void					calculate_health_bar_dimensions(t_game *game,
							t_health_bar *bar);
void					calculate_perpendicular(t_game *game, float *perp_x,
							float *perp_y);
void					calculate_rotation(t_game *game, float angle);
void					calculate_sky_range(t_sky_render *sky);
void					calculate_total_weight(float *weights, int n_text,
							float *total_weight);
void					calculate_vector(t_game *game, int enemy_index,
							float *dx, float *dy);
void					calculate_widths(t_game *game, float distance,
							int *total_width, int *white_width);
...
void					draw_crosshair_line(t_game *game, t_vector2d start,
							t_vector2d end);
void					draw_dot(t_game *game, t_vector2d center);
void					draw_extract_stripe(t_game *game, int s, int start_y,
							float d);
...
void					handle_corner_case(t_game *game, t_ray *ray, int next_x,
							int next_y);
void					handle_enemy_stop(t_enemy *enemy,
							unsigned long long *enemy_seed);
void					handle_epsilon_case(t_game *game, t_ray *ray);
void					handle_frame_specific_actions(t_game *game,
							int current_frame);
...
void					increment_and_update(t_game *game, int *current_task,
...
void					print_barrage_loc(t_vector2d new_position,
							t_vector2d base_position);
void					randomize_barrage_location(t_game *game);
void					randomize_single_supply(t_game *game,
							t_supplies *supply);
...
void					render_extract_multi_tile(t_game *game,
							t_vector2d base);
void					render_extract_ray(t_sprite_render_context *ctx);
void					render_floor(t_game *game);
void					render_floor_line(t_game *game, t_floor_ray *floor_ray,
							int y);
...
void					render_napalm_sprites(t_game *game,
							int current_frames[]);
...
void					render_pixel(t_game *game, t_texture *texture,
							t_vector2d screen);
...
void					render_single_tile(t_game *game, t_vector2d tile_pos,
							int tex_index);
void					render_sky(t_game *game);
void					render_sky_line(t_sky_render *sky, int i);
void					render_sky_pixel(t_game *game, t_vector2d pos,
							t_vector2d tex);
void					render_sky_rgb(t_game *game, int color);
void					render_sprite(t_sprite_render_context *ctx);
void					render_sprite_common(t_game *game, t_vector2d position,
							t_texture *texture);
...
void					run_raycasting_calculation_1(t_game *game,
							t_ray_node *current);
void					run_raycasting_calculation_2(t_game *game,
							t_ray_node *current);
void					run_raycasting_calculation_3(t_game *game,
							t_ray_node *current);
void					run_raycasting_calculations(t_game *game,
							t_ray_node *current);
...
void					script_napalm_enemies(t_game *game, int strike_no,
							float radius);
void					script_napalm_player(t_game *game, int strike_no,
							float radius);
void					script_skip_enter(t_game *game);
void					script_strike_enemies(t_game *game, int strike_no,
							float radius);
...
void					set_texture_weights(t_game *game, float *weights,
							float total_weight);
void					set_wall_face(t_ray *ray);
void					setup_barrage_sprite_context(\
...
void					setup_napalm_sprite_context(\
							t_sprite_render_context *ctx,
							t_game *game, t_vector2d offset,
							t_texture *texture);
...
void					transf_barr_sprite(t_game *game, t_vector2d sprite,
							t_vector2d *transform);
...
void					upd_prog_bar(t_game *game, int current, int total,
...
void					update_frame_and_momentum(t_enemy *enemy, int direction,
							int move);
void					update_gun_state(t_game *game);
void					update_normal_gun_frame(t_game *game);
void					update_player_position(t_game *game, float new_x,
							float new_y);
