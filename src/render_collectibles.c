...
int	get_pixel_color(t_sprite_render_context *ctx, t_vector2d tex)
{
	...
	if (tex.x < 0 || tex.x >= ctx->texture->width || tex.y < 0 || tex.y >= ctx->texture->height)
		return (-1);
	pixel = ctx->texture->data + ((int)tex.y * ctx->texture->tex_line_len + (int)tex.x * (ctx->texture->tex_bpp / 8));
...
void	transf_sprite(t_sprite_render_context *ctx)
{
	...
	inv_det = 1.0f / (ctx->game->player->plane.x * ctx->game->player->direction.y
		- ctx->game->player->direction.x * ctx->game->player->plane.y);
	ctx->calc.transform.x = inv_det * (ctx->game->player->direction.y * ctx->calc.sprite.x
		- ctx->game->player->direction.x * ctx->calc.sprite.y);
	ctx->calc.transform.y = inv_det * (-ctx->game->player->plane.y * ctx->calc.sprite.x
...
void	calc_sprite_height(t_sprite_render_context *ctx)
{
	...
	pitch_offset = (int)(ctx->game->player->pitch * ctx->game->screen_height);
	height_offset = (int)(ctx->game->player->height * ctx->game->screen_height / ctx->calc.transform.y);
...
void	draw_sprite_stripe(t_sprite_render_context *ctx, int stripe)
{
	...
	tex.x = (int)(256 * (stripe - (-ctx->calc.sprite_width / 2 + ctx->calc.sprite_screen_x))
		* ctx->texture->width / ctx->calc.sprite_width) / 256;
	...
	if (current && ctx->calc.transform.y < current->ray.perpwalldist)
	{
		...
		while (y < ctx->calc.draw_end_y)
		{
			pitch_offset = (int)(ctx->game->player->pitch * ctx->game->screen_height);
			height_offset = (int)(ctx->game->player->height * ctx->game->screen_height / ctx->calc.transform.y);
			d = (y - pitch_offset - height_offset) * 256 - ctx->game->screen_height * 128 + ctx->calc.sprite_height * 128;
			tex.y = ((d * ctx->texture->height) / ctx->calc.sprite_height) / 256;
			if (tex.y < 0)
				tex.y = 0;
