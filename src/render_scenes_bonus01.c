...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/23 17:57:38 by vitenner          #+#    #+#             */
/*   Updated: 2024/09/23 17:57:38 by vitenner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_first_loop(t_game *game)
{
	...
	if (frame_to_render >= T_N_INTRO1 - 1)
...
int	handle_second_loop(t_game *game)
{
	...
	frame_to_render = game->opening_state.randomized_frames[
		elapsed_frames % (T_N_INTRO1 - SECOND_LOOP_START)];
	if (elapsed_frames >= T_N_INTRO1 - SECOND_LOOP_START)
	{
...
void	render_current_frame(t_game *game, int frame_to_render)
{
	...
	opening_texture = &game->t_intro1[frame_to_render];
