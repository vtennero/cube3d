...
{
	bar->width = game->screen_width / 4; // 1/4 of the screen width
...
void draw_health_bar_fill(t_game *game, t_health_bar *bar)
{
	...
	fill_width = (game->player->hp * (bar->width - 2 * HEALTH_BAR_BORDER))
		/ MAX_HEALTH;
	for (int y = bar->pos.y + HEALTH_BAR_BORDER; y < bar->pos.y + bar->height - HEALTH_BAR_BORDER; y++)
	{
		for (int x = bar->pos.x + HEALTH_BAR_BORDER; x < bar->pos.x + HEALTH_BAR_BORDER + fill_width; x++)
		{
